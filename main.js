...
const bar = new cliProgress.SingleBar({
  format: "|" + colors.cyan('{bar}') + '| {percentage}% || ETA: {eta}s || {value}/{total} stories || Current: "{storyTitle}" (segment: {segment})',
...
async function insertAllStoryData() {
  ...
  Promise.map(storyObjects, async (stories) => {
    bar.start(endStory - startStory, 0);
    bar.update(bar.getProgress(), { storyTitle: "None", segment: "Fetching Stories" })

    await Promise.map(stories, async (storyObject) => {
      try {
        bar.update({ storyTitle: storyObject.title.rendered, segment: "Inserting Content" })
      ...
      try {
        bar.update({ storyTitle: storyObject.title.rendered, segment: "Inserting Tags" })
      ...
      try {
        bar.update({ storyTitle: storyObject.title.rendered, segment: "Inserting Authors" })
